...
epochNum = 10
lossFileName = "lossEpoch10.txt"
validLossFileName = "validLossEpoch10.txt"
modelName = "translationModel10.pth"
...
# 创建数据加载器
train_loader = DataLoader(train_dataset, batch_size=200, collate_fn=lambda x: collate_fn(x, tokenizer))
val_loader = DataLoader(val_dataset, batch_size=200, collate_fn=lambda x: collate_fn(x, tokenizer))
test_loader = DataLoader(test_dataset, batch_size=1, collate_fn=lambda x: collate_fn(x, tokenizer))
# 设置模型和优化器
device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
num_hiddens, ffn_num_hiddens, num_heads, num_blks = 256, 1024, 8, 4
model = Transformer(tokenizer.vocab_size, num_hiddens, ffn_num_hiddens, num_heads, num_blks).to(device)
...
for epoch in range(epochNum):
    ...
    for src_batch, src_mask, tgt_batch, tgt_mask in train_loader:
        optimizer.zero_grad()
        src_batch, src_mask, tgt_batch, tgt_mask = src_batch.to(device), src_mask.to(device), tgt_batch.to(
            device), tgt_mask.to(device)
        output = model(src_batch, tgt_batch[:, :-1], src_mask)[0]
        active_loss = tgt_mask[:, :-1].reshape(-1) == 1
        active_logits = output.view(-1, len(tokenizer.vocab))[active_loss]
    ...
    with torch.no_grad():
        ...
        validLossList.append(val_loss / len(val_loader))
...
evaluate_bleu(model, test_loader, tokenizer)
print(lossList)
print(validLossList)
with open(lossFileName, "w") as file:
    for number in lossList:
        file.write(f"{number}\n")
with open(validLossFileName, "w") as file:
    for number in validLossList:
        file.write(f"{number}\n")
