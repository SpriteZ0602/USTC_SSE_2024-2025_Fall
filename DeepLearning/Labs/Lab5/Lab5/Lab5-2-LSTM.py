...
class Model(nn.Module):
    def __init__(self):
        ...
        self.LSTM = nn.LSTM(input_size=1, hidden_size=20, num_layers=2, batch_first=True, dropout=0.2)
...
def init_hidden(batch_size):
    # 初始化 hidden state 和 cell state，大小均为 (num_layers, batch_size, hidden_size)
    return (torch.zeros(2, batch_size, 20),  # hidden state
            torch.zeros(2, batch_size, 20))  # cell state
